...
- The C# you show must match the actual starter scripts — READ the real files before writing (paths below). Use the real field/method names (xSpeed, maxAttackRange, ChangeHealth, StatManager.manager, etc.). If a lesson invents something the starter does not have (potions, ladders, boss), write plausible NEW code consistent with the starter's style: MonoBehaviour, [SerializeField] private, private helper methods, Debug.Log, CompareTag, Vector3/Vector2, Time.deltaTime.
...
- `cs-curriculum-main/Assets/Scripts/PlayerController.cs` — xSpeed/ySpeed/jumpForce [SerializeField]; Input.GetAxis; xVector = new Vector2(xDirection * xSpeed * Time.deltaTime, 0); transform.Translate; isPlatformer decided by scene name in Start (Platformer or Start → true; gravityScale 2 else 0); Space → rb.AddForce(Vector2.up * jumpForce, ForceMode2D.Impulse); E key → finds every Lever, Interact() if Vector3.Distance < 2.0; UpdateAnimation sets anim.SetBool("IsWalking"), anim.SetInteger("WalkDir", 0=up/1=side/2=down), sprite.flipX, anim.SetTrigger("Attack") on mouse 0; OnTriggerEnter2D → other.CompareTag("Coin") → StatManager.manager.ChangeCoins(1), Destroy(other.gameObject); OnCollisionEnter2D → collision.gameObject.CompareTag("Spikes") → ChangeHealth(-1).
- `cs-curriculum-main/Assets/Scripts/StatManager.cs` — public static StatManager manager singleton (Awake, DontDestroyOnLoad); [SerializeField] int health = 3, coins = 0; ChangeHealth(int amount) logs, health<=0 → HandleDeath (coins halved with FloorToInt, health reset 3, LoadScene("Start")); ChangeCoins(int amount) logs. Start() logs "Game Started! Health: 3 | Coins: 0".
- `cs-curriculum-main/Assets/Scripts/TurretController.cs` — [SerializeField] maxAttackRange 7f, attackCooldown 2f, projectilePrefab, projectileSpawnPoint; finds PlayerController by tag in Start; Update: Vector3.Distance ≤ maxAttackRange AND Time.time >= lastAttackTime + attackCooldown → AttackPlayer(): Instantiate(projectilePrefab, spawn point, Quaternion.LookRotation(Vector3.forward, to-player)) then Projectile.Initialize(player position).
- `cs-curriculum-main/Assets/Scripts/Projectile.cs` — READ IT.
- `cs-curriculum-main/Assets/Scripts/EnemyController.cs` — [SerializeField] speed 2, damage 1, patrolDistance 3, chaseDistance 5; isPlatformer by scene name; PlatformerMove(): translate right*direction*speed*deltaTime, flip when distance from startPosition > patrolDistance; TopDownMove(): if dist < chaseDistance → move toward player normalized; TryDamage() with damageCooldown 1.0 (lastDamageTime); OnCollisionEnter2D/Stay2D with Player tag.
- `cs-curriculum-main/Assets/Scripts/Lever.cs` — sprite swap offSprite/onSprite; UnityEvent onLeverOn/onLeverOff; Interact() toggles isOn and invokes events.
- Scenes: `Assets/_Scenes` = Start, Overworld, Platformer. Prefabs in `Assets/Prefabs`: Coin, Spikes, Player, Turret, Turret_Projectile, MobileEnemy, AxeItem, Switch. Artwork: Top_Down (WallsTileSet, TP Grass) and Platformer (CavesTileSet, CaveEntranceTileSet) + PlayerAnimator (Axe/Shovel variants).
...
- 1.2a Editor tour: five panes (Hierarchy/Scene/Game/Inspector/Project), GameObject vs Component vs Transform, play mode vs edit mode (edits in play mode vanish — demo it), prefabs intro, saving scenes.
- 1.3a First script: create PlayerLogger.cs; class : MonoBehaviour; Update() runs every frame; Debug.Log; [SerializeField] private float; attach to Player; Console. Bug hunt: typo class name, forgetting to attach, infinite log spam in Update.
- 1.5a Tuning: xSpeed/ySpeed in Inspector vs code defaults; the serialized-vs-code precedence; tuning pass on feel; why Vector2 not Vector3 for a 2D game (transform.Translate takes Vector3 implicitly).
- 1.6a Animation states: Animator window on Player; parameters IsWalking (bool), WalkDir (int 0 up/1 side/2 down), Attack (trigger); transitions; sprite.flipX; where UpdateAnimation fits; bug hunt: missing transition, param never set.

Unit 2 — Stats & Pickups (Sept-Oct):
- 2.1a Coins: OnTriggerEnter2D on Player; other.CompareTag("Coin"); StatManager.manager.ChangeCoins(1); Destroy(other.gameObject). Predict-first: why Destroy the coin and not the player. Debug.Log shows the count — print statements as your first debugger. Trap: CompareTag string typo silently never fires.
- 2.2a Spikes: OnCollisionEnter2D vs trigger (spikes have solid collider, coins trigger); ChangeHealth(-1); death path in StatManager: halve coins, reset, LoadScene("Start") — read HandleDeath together; socratic: why does the StatManager survive scene loads (DontDestroyOnLoad) and why must it be a singleton.
- 2.3a Potions: NEW pickup that heals; cap at max health 3 — the first real if statement (selection!); design: heal +1 vs +2; cap with if or Mathf.Min; bug hunt: overheal, negative health display.
- 2.4a Prefabs & spawning: Coin/Spikes prefabs exist — edit prefab vs instance (blue bar); place many; playtest discipline: fix one thing at a time, note hypothesis first; intro playtest sheet used all year.

Unit 3 — The Turret (Oct):
- 3.1a Detection radius: Vector3.Distance(a, b); maxAttackRange 7f; predict: what happens at exactly 7.0; debug with Debug.Log(distance) and draw the radius mentally; AP: abstraction of distance.
- 3.2a Projectiles: Instantiate(prefab, spawnPoint.position, rotation); Quaternion.LookRotation(Vector3.forward, target - origin); Projectile.Initialize(target position) — READ Projectile.cs first; Destroy on arrival/hit; the prefab must carry the Projectile script.
- 3.3a Cooldowns: attackCooldown 2f; lastAttackTime pattern (Time.time >= last + cooldown → fire, update last); why not "fire every frame"; timers as state.
- 3.4a Damage + playtest: projectile hits player → ChangeHealth(-1); tuning pass (range/cooldown/speed); playtest with a partner; reflect on difficulty curves.
